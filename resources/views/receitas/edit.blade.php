    ...
    <div class="form-card">
        ...
        <form action="{{ route('receitas.update', $receita) }}" method="POST" class="recipe-form">
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="titulo" name="titulo"
                        ...
                        value="{{ old('titulo', $receita->titulo) }}" placeholder="Título da receita">
                    @error('titulo')
                ...
                <div class="form-group">
                    ...
                    <textarea id="descricao" name="descricao" class="form-control @error('descricao') is-invalid @enderror" rows="4"
                        placeholder="Descrição da receita">{{ old('descricao', $receita->descricao) }}</textarea>
                    @error('descricao')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="tempo_preparo" name="tempo_preparo"
                            ...
                            value="{{ old('tempo_preparo', $receita->tempo_preparo) }}" placeholder="Ex: 45 minutos">
                        @error('tempo_preparo')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="dificuldade" name="dificuldade"
                            class="form-control form-select @error('dificuldade') is-invalid @enderror">
                            <option value="">Selecione...</option>
                            <option value="fácil"
                                {{ old('dificuldade', $receita->dificuldade) == 'fácil' ? 'selected' : '' }}>🟢 Fácil
                            </option>
                            <option value="médio"
                                {{ old('dificuldade', $receita->dificuldade) == 'médio' ? 'selected' : '' }}>🟡 Médio
                            </option>
                            <option value="difícil"
                                {{ old('dificuldade', $receita->dificuldade) == 'difícil' ? 'selected' : '' }}>🔴 Difícil
                            </option>
                        </select>
                        @error('dificuldade')
                            <span class="invalid-feedback">{{ $message }}</span>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="custo_medio" name="custo_medio" step="0.01" min="0"
                            ...
                            value="{{ old('custo_medio', $receita->custo_medio) }}" placeholder="0,00">
                        @error('custo_medio')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="form-group">
                    <label class="form-label" for="endereco_video">Link do Vídeo (YouTube)</label>
                    ...
                    <div id="video-preview" class="video-preview mt-2" style="display:none">
                        <div class="video-preview__label">
                            <span>▶ Preview do vídeo</span>
                            <button type="button" id="btn-fechar-preview" class="video-preview__fechar">✕
                                Fechar</button>
                        </div>
                    ...
                    <ul class="ingredient-list mb-3">
                        @foreach ($receita->ingredientes as $ingrediente)
                            <li class="ingredient-item">
                                ...
                                <form action="{{ route('receitas.ingredientes.destroy', [$receita, $ingrediente]) }}"
                                    method="POST">
                                    @csrf @method('DELETE')
                    ...
                    <div class="row g-2">
                        <div class="col-7">
                            <input type="text" name="nome"
                                ...
                                placeholder="Nome do ingrediente *" value="{{ old('nome') }}">
                            @error('nome')
            ...
            <div class="form-card h-100">
                ...
                <form action="{{ route('receitas.passos.store', $receita) }}" method="POST">
                    ...
                    <textarea name="descricao" rows="3"
                        ...
                        placeholder="Descreva este passo do preparo... *">{{ old('descricao') }}</textarea>
                    @error('descricao')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
